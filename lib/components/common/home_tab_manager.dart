...
class HomeTabManager {
  static const TabBottomShape bottomShape = TabBottomShape.rounded; // TabBar 전체의 bottomShape
  ...
  static final List<TabItem> tabItems = [
    ...
    const TabItem(
      label: 'AI매매신호',
      content: AISignalTabContent(),
    ),
    const TabItem(
      label: '종목캐치',
      content: StockCatchTabContent(),
    ),
    const TabItem(
      label: '마켓뷰',
      content: MarketViewTabContent(),
    ),
  ];
  
  static List<String> get tabNames => tabItems.map((item) => item.label).toList();
